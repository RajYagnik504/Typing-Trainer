...
const STAGES = [
  { id: 4, name: 'Number Row Extension' },
  { id: 5, name: 'Symbols & Specials' }
];

export default function SymbolsModule() {
  ...
  const engine = useTypingEngine(targetText, ({ accuracy }) => {
    setPhase('result');
    if (accuracy >= 90) {
      unlockAchievement(`symbols_stage_${selectedStage}`, 'Symbol Master', `Mastered Stage ${selectedStage} of Numbers & Symbols`, '🔢');
    }
  });
  ...
  return (
    <div className="glass-panel animate-fadeIn" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%', alignItems: 'center' }}>
      <div style={{ textAlign: 'center', maxWidth: '560px' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 900, color: 'white', marginBottom: '0.5rem' }}>🔢 Numbers & Symbols</h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
          Extend spatial coordination to number keys, shifts, brackets, and special punctuation. Critical practice for programmers and technical writers.
        </p>
      </div>
      ...
      {phase === 'result' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', alignItems: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
            {engine.accuracyPct >= 90 ? 'Excellent accuracy! You have solid spatial reach.' : 'Take your time, symbols require careful coordinates.'}
          </p>
