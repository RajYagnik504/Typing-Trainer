...
export default function BottomRowModule() {
  ...
  const targetText = generateMasteryText(3);
  const engine = useTypingEngine(targetText, ({ accuracy }) => {
    setPhase('result');
    if (accuracy >= 90) {
      unlockAchievement('bottomrow_master', 'Bottom Row Master', 'Mastered the bottom row keys', '🛹');
    }
  });
  ...
  return (
    <div className="glass-panel animate-fadeIn" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%', alignItems: 'center' }}>
      <div style={{ textAlign: 'center', maxWidth: '560px' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 900, color: 'white', marginBottom: '0.5rem' }}>🛹 Bottom Row Curl</h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
          Curl fingers downwards to master the ZXCVBNM keys. Keep wrist motion to an absolute minimum for maximum efficiency.
        </p>
      </div>
      ...
      {phase === 'select' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'center' }}>
          <div style={{ background: 'rgba(255,255,255,0.03)', padding: '1rem 2rem', borderRadius: '8px', border: '1px solid var(--glass-border)' }}>
            ...
            <span style={{ fontFamily: 'monospace', color: 'white', fontWeight: 700 }}>Z X C V B N M</span>
          </div>
          <button 
            onClick={handleStart}
            style={{
      ...
      {phase === 'result' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', alignItems: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
            {engine.accuracyPct >= 90 ? 'Terrific curl extension! Your spatial mappings are fully forming.' : 'Focus on the bottom row curls and try again.'}
          </p>
